...
bool LD6002::hasNewBreathPhase() const { return newBreathPhase; }
...
float LD6002::getBreathPhase() const {
  return breathPhase;
}
...
void LD6002::clearBreathPhaseFlag() {
  newBreathPhase = false;
}
...
void LD6002::parseFrame(const uint8_t *frame) {
  ...
  switch (type) {
    ...
    {
      if (calcXorInverse(frame + 8, 12) != frame[20]) return;
      // Payload: total phase, breathing phase, heartbeat phase (3 floats).
      breathPhase = bytesToFloat(&frame[12]);
      newBreathPhase = true;
      break;
    }
